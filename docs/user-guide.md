# Trillo SRE Copilot — User Guide

Investigate a **Trillo Observability** deployment from inside Claude
...
Trillo Observability watches a fleet of AI agents through their OpenTelemetry data — reliability,
...
Claude runs the right investigation steps against Trillo Observability and comes back with a
...
- **Read-only against the fleet.** It reads findings, failure clusters, traces,
  ...
  app.
- **One write:** it can file an **investigation report** back into Trillo Observability, which
...
`/sre:cost-spike-investigation`) or just describe the problem and let Claude choose.
...
verdict. The report lands in Trillo Observability tagged as copilot-authored, with your name as the
...
- **Connection not listed** → confirm the plugin is installed
  (`claude plugin install sre`) and, if self-hosted, that `SRE_MCP_URL` is set.

---

## Appendix — for admins enabling the plugin

Two things gate what SREs can do; both live on the Trillo Observability side, not in the plugin:

1. **Expose the app.** The Trillo Observability app's `appName` must be on the catalog allow-list,
...
Nothing an admin does requires a plugin change: as Trillo Observability exposes more tools, the
