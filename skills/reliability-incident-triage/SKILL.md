...
1. `getTopPlatformFindings` filtered to reliability / `status = open` (and a time window
...
3. `getFailureClusterStatistics(findingId)` → counts by **status / agent / app** and
   ...
   - the failing node is a **shared tool/system across multiple agents** ⇒
     **likely DEPENDENCY**.
4. `getImpactedAgentFindings(findingId)` → for a DEPENDENCY case, the other agents
...
8. `getAgentPerformanceBaseline(agentId, metric=error_rate)` → is this a step change vs the
...
- If `getFailureClusterStatistics` shows wide cross-instance spread, **lead with the
